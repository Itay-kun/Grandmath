...
function getSelectedVoice(all_voices = voices){
  //update it to be more dinamic with input vars
	// let all_voices = speechSynthesis.getVoices().filter(function (voice) { return voice.lang == "en-US"})
	let index = document.querySelector("#voice").selectedIndex
	let selected = all_voices[index]
...
function formToJson(formElement = document.forms[0]) {
  ...
  settings["selected_voice_id"] = getSelectedVoice()
  return settings;
}
...
document.addEventListener("focusin", function() {
  addOperatorsIcons();
  wait(500).then(addVoicesOptions(voices));
});
